...
public class CenasMaradasInitRegistrationEventTemplate extends WriteCustomTask {

    @Override
    public void runTask() throws Exception {
        final Set<Registration> set = Bennu.getInstance().getRegistrationsSet().stream()
                .filter(registration -> registration.getEventTemplate() == null)
                .filter(registration -> registration.getSourceRegistration() != null)
                .filter(registration -> registration.getStartDate().getYear() == 2022)
