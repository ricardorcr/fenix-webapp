...
public class CheckSurveyResponses extends ReadCustomTask implements RemoteReader {
        ...
        try {
            FenixFramework.atomic(() -> {
                if (Survey.survey(application, survey.get("surveyId").getAsString()) == null) {
                    taskLog("Add survey for: " + application.getExternalId());
                    Survey.addSurvey(application, survey);
                } else {
                    taskLog("Check survey response for: " + application.getExternalId());
