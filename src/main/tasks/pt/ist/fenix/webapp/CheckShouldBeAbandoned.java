    ...
    public void runTask() throws Exception {
        ...
        for (final DegreeCurricularPlan degreeCurricularPlan : DegreeCurricularPlan.readBolonhaDegreeCurricularPlans()) {
            ...
            if (degreeCurricularPlan.hasExecutionDegreeFor(getWhenToAbandon().getExecutionYear())
                ...
                for (final StudentCurricularPlan scp : degreeCurricularPlan.getActiveStudentCurricularPlans()) {
                    try {
                        processStudent(scp);
                        changedForAbandoned++;
                    } catch (Exception e) {
                       taskLog("Aluno %s  - %s devia estar em abandono%n", scp.getPerson().getUsername(), scp.getDegreeCurricularPlan().getName());
                       e.printStackTrace();
                    }
                }
            }
        ...
        taskLog("Estados mudados: %s%n", changedForAbandoned);
