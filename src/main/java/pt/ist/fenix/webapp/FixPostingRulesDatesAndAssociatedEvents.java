...
public class FixPostingRulesDatesAndAssociatedEvents extends CustomTask {
    ...
    public void runTask() throws Exception {
        ...
        Set<DegreeCurricularPlan> curricularPlans = Bennu.getInstance().getDegreeCurricularPlansSet().stream()
                ...
                .filter(DegreeCurricularPlan::isBolonhaDegree)
                .filter(dcp -> dcp.isFirstCycle() || dcp.isSecondCycle())
                .filter(DegreeCurricularPlan::isActive)
    ...
    private void fix(final DegreeCurricularPlan dcp) throws Exception {
        Set<PostingRule> postingRulesPRE = getPostingRuleByEventTypeAndDate(
                dcp.getServiceAgreementTemplate(), EventType.PARTIAL_REGIME_ENROLMENT_GRATUITY, START_DATE);

        Set<PostingRule> postingRulesPR = getPostingRuleByEventTypeAndDate(
                dcp.getServiceAgreementTemplate(), EventType.PARTIAL_REGIME_GRATUITY, START_DATE);

        if (postingRulesPRE.size() > 2 || postingRulesPR.size() > 2) {
            taskLog("%n%s%n", dcp.getName());
            taskLog("Para tempo parcial por unidade temos: %s%n", postingRulesPRE.size());
            taskLog("Para tempo parcial parte fixa temos: %s%n", postingRulesPR.size());

            FenixFramework.atomic(() -> {
                treatPRE(postingRulesPRE);
            });
            FenixFramework.atomic(() -> {
                treatPR(postingRulesPR);
            });
        }
    }

    private void treatPRE(final Set<PostingRule> postingRulesPRE) {
        taskLog("##### Partial Regime Enrolment #####");
        Set<EnrolmentGratuityPR> prSet = postingRulesPRE.stream()
        ...
        if (prSet.size() == 2) {
            ...
            if (isAlien != null && isNormal != null) {
                ...
                Set<PostingRule> prToFix = new HashSet<>(postingRulesPRE);
    ...
    private void treatPR(final Set<PostingRule> postingRulesPR) {
        taskLog("##### Partial Regime #####");
        Set<PartialRegimePR> prSet = postingRulesPR.stream()
                .filter(pr -> pr.getEndDate() == null)
                .map(PartialRegimePR.class::cast)
                .collect(Collectors.toSet());
        if (prSet.size() == 2) {
            //is ok
            PartialRegimePR isAlien = null;
            PartialRegimePR isNormal = null;
            Iterator<PartialRegimePR> iterator = prSet.iterator();
            while (iterator.hasNext()) {
                PartialRegimePR next = iterator.next();
                if (next.isForAliens()) {
                    isAlien = next;
                } else {
                    isNormal = next;
            ...
            if (isAlien != null && isNormal != null) {
                ...
                taskLog("Vou mudar a data de inicio da PR Normal %s de: %s para: %s%n",
                        isNormal.getExternalId(), isNormal.getStartDate(), START_DATE.toDateTimeAtStartOfDay());
                changeStartDate(isAlien, isNormal);
                Set<PostingRule> prToFix = new HashSet<>(postingRulesPR);
                prToFix.removeAll(prSet);
                Map<PostingRule, String> prMap = new HashMap<>();
                for (PostingRule pr : prToFix) {
                    String type = fixPrDates(pr);
                ...
                for (PostingRule pr : prToFix) {
                    fix((PartialRegimePR) pr, prMap.get(pr), isAlien, isNormal);
                }
            } else {
                taskLog("ERRO - Tem 2 mas não batem certo");
            }
    ...
    private void fix(final PartialRegimePR postingRule, final String type, final PartialRegimePR alien, final PartialRegimePR normal) {
        if (SAME_DAY.equals(type)) {
            if (postingRule.getEnrolmentEvaluationEventSet().isEmpty() &&
                    postingRule.getPartialRegimeEventSet().isEmpty()) {
                taskLog("Deleting PR 1 %s%n", postingRule.getExternalId());
                postingRule.delete();
            } else {
                fixEvents(postingRule, alien, normal, true);
                taskLog("Deleting PR 2 %s%n", postingRule.getExternalId());
                postingRule.delete();
            }
        } else if (LAST_YEAR.equals(type)) {
            fixEvents(postingRule, alien, normal, false);
    ...
    private void fixEvents(final PartialRegimePR oldPr, final PartialRegimePR alien, final PartialRegimePR normal, final boolean changePR) {
        if (!oldPr.getEnrolmentEvaluationEventSet().isEmpty()) {
            taskLog("ERRO - Mau maria");
        }

        for (PartialRegimeEvent event : oldPr.getPartialRegimeEventSet()) {
            if (event.getWhenOccured().isAfter(START_DATE.toDateTimeAtStartOfDay())) {
                if (event.getExecutionYear() == ExecutionYear.readCurrentExecutionYear()) {
                    boolean isAlien = new IsAlienRule().isAppliableFor(event.getStudentCurricularPlan(), event.getExecutionYear());
                    if (!event.isCancelled()) {
                        if (isAlien) {
                            fixEvent(event, alien, changePR);
                        ...
                        if (changePR) {
                            //we still want to delete the wrongPR, although nothing will be done to the event
                            setPostingRule(event, isAlien ? alien : normal);
                        }
                    }
                }
            }
                ...
                if (!calculator.hasDueDebtAmount()) {
                    if (oldEvent instanceof EnrolmentGratuityEvent) {
                        ...
                        if (amountToPay.getAmount().compareTo(calculator.getDebtAmount()) == 0) {
                            ...
                            taskLog("£££££££ %s - %s : %s%n", amountToPay.getAmount(), calculator.getDebtAmount(),
                                    amountToPay.getAmount().compareTo(calculator.getDebtAmount()));
                        }
                    } else {
                        PartialRegimePR pr = (PartialRegimePR) newPostingRule;
                        if (pr.getAmount().getAmount().compareTo(calculator.getDebtAmount()) == 0) {
                            return true;
                        } else {
                            taskLog("£££££££ %s - %s : %s%n", pr.getAmount().getAmount(), calculator.getDebtAmount(),
                                    pr.getAmount().getAmount().compareTo(calculator.getDebtAmount()));
                        }
                    }
                }
...
//                }
    ...
    private boolean setPostingRule(final Event event, final PostingRule postingRule) {
        Method[] methodsSetPR = null;
        boolean needToChangeEvent = true;
        if (event instanceof PartialRegimeEvent) {
            methodsSetPR = getAllMethodsInHierarchy(PartialRegimeEvent.class);
            PartialRegimePR oldPr = (PartialRegimePR) event.getPostingRule();
            PartialRegimePR newPr = (PartialRegimePR) postingRule;
            if (oldPr.getAmount().equals(newPr.getAmount())
            ...
            methodsSetPR = getAllMethodsInHierarchy(EnrolmentGratuityEvent.class);
