...
//                .filter(studentPlan -> studentPlan.getStudent().getPerson().getUsername().equals("ist171070"))
        ...
        for (Registration registration : studentMap.keySet()) {
            try {
                ...
                e.printStackTrace();
                if (e.getMessage().equals("error.cannot.transition.plan.no.available.cycles.to.transition")) {
                    checkAndCreateNewCycle(registration);
                }
            }
        }
    ...
    private void checkAndCreateNewCycle(Registration registration) {
        ...
        if (studentTransitionPlans.size() == 1) {
            ...
            if (scp.getCycleCurriculumGroups().size() == 1) {
                scp.getCycleCurriculumGroups().stream()
                        .filter(ccg -> ccg.getCurriculumLines().isEmpty() || ccg.getCurriculumLines().stream().allMatch(cl -> {
                            if (cl.isEnrolment()) {
                                Enrolment enrolment = (Enrolment) cl;
                                return enrolment.isNotEvaluated() || enrolment.isFlunked();
                            } else {
                                return false;
                            }
                        }))
                        .filter(ccg -> ccg.getDegreeCurricularPlanOfDegreeModule() == studentPlan.getDegreeCurricularTransitionPlan().getOriginDegreeCurricularPlan())
                        .forEach(ccg -> {
                            //createNewPlan(registration, scp, ccg.getCycleType(), studentPlan.getDegreeCurricularTransitionPlan());
                            taskLog("Created new plan for: %s\t%s - origin%n", registration.getPerson().getUsername(), scp.getDegree().getSigla());
                        });
            }
        }
    }

    private void createNewPlan(Registration registration, StudentCurricularPlan scp, CycleType cycleType, DegreeCurricularTransitionPlan transitionPlan) {
        FenixFramework.atomic(() -> {
            if (scp.getDegreeCurricularPlan().getDegree() == transitionPlan.getDestinationDegreeCurricularPlan().getDegree()) {
                StudentCurricularPlan.createBolonhaStudentCurricularPlan(registration, transitionPlan.getDestinationDegreeCurricularPlan(), new YearMonthDay(),
                        transitionPlan.getDestinationExecutionYear().getFirstExecutionPeriod());
            } else {
                final Registration newRegistration = new Registration(registration.getPerson(), transitionPlan.getDestinationDegreeCurricularPlan(),
                        registration.getRegistrationProtocol(), cycleType, transitionPlan.getDestinationExecutionYear());
                newRegistration.setIngressionType(registration.getIngressionType());
            }
        });
    }
        ...
        if (studentTransitionPlans.size() == 1) {
            ...
            if (!scp.getDegree().isSecondCycle() && scp.getSecondCycle() != null) { //tem 2º ciclo em avanço
                if (scp.getFirstCycle() != null && scp.getFirstCycle().isConcluded()) {
                    ...
                    taskLog(" -> 2º ciclo em avanço e falhou nas condições");
                ...
                if (scp.hasConcludedCycle(CycleType.FIRST_CYCLE)) {
                    ...
                    taskLog(" -> mestrado integrado e falhou nas condições");
                    return;
                }
            }
...
//                            "Lamentamos o atraso na conclusão desta operação.\n" +
    ...
    private void sendDebtEmail(Registration registration) {
        FenixFramework.atomic(() -> {
            Message.fromSystem()
                    .to(Group.users(registration.getPerson().getUser()))
                    .subject("Transição Curricular Pendente")
                    .textBody("Caro(a) aluno(a),\n\n" +
                            "Não foi possível proceder à sua transição conforme estava planeado pois tem dívida(s) por saldar.\n" +
                            "Após saldar a(s) dívida(s) o plano fica em condições de ser transitado, não necessitando de mais " +
                            "nenhuma ação sua.\n" +
                            "\n" +
                            "Os melhores cumprimentos,\n" +
                            "A Equipa FenixEdu")
