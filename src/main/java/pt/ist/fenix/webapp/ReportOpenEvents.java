...
public class ReportOpenEvents extends CustomTask {
    ...
    public void runTask() throws Exception {
        ...
        results2.forEach(o -> {
            ...
            row.setCell("Event", (String) o[0]);
            row.setCell("Document Number", (String) o[1]);
            row.setCell("Document Type", (String) o[2]);
            row.setCell("Value", (String) o[3]);
            row.setCell("Advancement", (String) o[4]);
            row.setCell("Is Initialization", (String) o[5]);
            row.setCell("Is Integrated", (String) o[6]);
            row.setCell("Is Annulment", (String) o[7]);
        });
        ...
        try {
            return FenixFramework.getTransactionManager().withTransaction(() -> {
                ...
                if (eventDate.isBefore(instant)) {
                    ...
                    final BigDecimal paiedDebt = calculator.getPayments()
                    ...
                    final BigDecimal dueAmount = debt.subtract(paiedDebt).subtract(exempt);
                    final BigDecimal dueInterest = calculator.getDueInterestAmount().add(calculator.getDueFineAmount());
