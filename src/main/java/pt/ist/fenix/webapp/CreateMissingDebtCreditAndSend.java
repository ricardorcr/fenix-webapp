...
public class CreateMissingDebtCreditAndSend extends SapCustomTask {
    ...
    protected void runTask(ErrorLogConsumer errorLogConsumer, EventLogger elogger) {
        SapRoot.getInstance().getSapRequestSet().stream()
                ...
                .filter(event -> !event.getSapRequestSet().stream()
                                    .filter(sr -> !sr.isInitialization())
                                    .anyMatch(sr -> sr.getRequestType() == SapRequestType.DEBT_CREDIT))
                .forEach(event -> process(event, elogger, errorLogConsumer));
    }

    private void process(final Event event, final EventLogger elogger, final ErrorLogConsumer errorLog) {
        ...
        if (amount.isPositive()) {
            ...
            final SapRequest debtCredit = registerDebtCredit(getCreditEntry(amount), event, true);
            debtCredit.setRequest(debtCredit.getRequest().replace("2022-02-08", "2021-12-31"));
